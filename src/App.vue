...
/* Запас под нижнее меню держит обёртка, а не каждый экран по отдельности:
   ...
   Отступа хватает экранам, которые растут вниз по содержимому. Экрану,
   который сам занимает ровно высоту окна (карта с панелью действий под ней),
   он не помогает: такой экран всё равно кончается там, где начинается меню,
   и нижняя полоса кнопок оказывается под ним. Поэтому высота меню объявлена
   переменной — такие экраны вычитают её сами: height: calc(100dvh -
   var(--tabbar-h, 0px)). Без меню переменной нет и вычитается ноль. */
