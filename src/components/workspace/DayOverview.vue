...
// на свете. Просрочка в день не поднимается: на главной её сотни, и день
// становился свалкой — для неё есть «Обновить даты» в задаче.
const SUB_HORIZON_DAYS = 7;

// Смещение срока от этого дня в целых сутках: 0 — сегодня, 1 — завтра.
// Считаем по календарным дням, а не по часам: иначе переход на зимнее время
// сдвигал бы всё на день.
...
// Сервер отдаёт окно с запасом по краям, окончательный отбор — здесь: где
// проходит граница суток, знает только браузер со своей таймзоной.
const visibleSubs = computed(() =>
  props.mainSubtasks
    .filter((sub) => {
      const offset = subDue(sub).offset;
      return offset !== null && offset >= 0 && offset <= SUB_HORIZON_DAYS;
    })
