<script setup>
// Запасной движок карты — Leaflet поверх бесплатных подложек.
//
// Работает без ключей: схема OSM, спутник Esri, рельеф OpenTopo. Основной
// движок — Google (GoogleTravelMap.vue), этот остаётся на случай, когда ключа
// нет или он перестал работать.
...
// Карту центрируем по стране один раз при открытии. Дальше вид принадлежит
// пользователю: добавление точки не должно отбрасывать его обратно на всю страну.
let fitted = false;
...
// Сравниваем границы по значению: вычисляемый массив пересоздаётся на каждой
// перерисовке, и слежение по ссылке сбрасывало бы вид после каждой правки.
