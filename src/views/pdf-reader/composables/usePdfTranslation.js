...
export function usePdfTranslation(fileName) {
    ...
    async function translate(text, src, tgt) {
        ...
        try {
            // Always use relative paths — nginx proxies /deepl-free/ and /deepl-pro/ in prod,
            // Vite proxies them in dev.
            const endpoints = ['/deepl-free/v2/translate', '/deepl-pro/v2/translate'];
    ...
    async function checkApiKey() {
        ...
        try {
            for (const endpoint of ['/deepl-free/v2/usage', '/deepl-pro/v2/usage']) {
                const res = await fetch(endpoint, {
    ...
    async function getGigaChatToken(authKey) {
        ...
        const res = await fetch('/gigachat-auth/api/v2/oauth', {
            ...
            headers: {
                ...
                'RqUID': (crypto.randomUUID ?? (() => ([1e7]+-1e3+-4e3+-8e3+-1e11).replace(/[018]/g, c => (c ^ (crypto.getRandomValues(new Uint8Array(1))[0] & (15 >> (c / 4)))).toString(16))))(),
            },
